...
- [x] T031 [P] [US3] Create `AppLoading` widget in `lib/core/widgets/app_loading.dart`. A `StatelessWidget`. Returns `Center(child: SpinKitFadingCircle(color: Theme.of(context).colorScheme.primary, size: 50.w))`. Import flutter*spinkit, flutter_screenutil, flutter/material.dart. Also create a static method `static void show(BuildContext context)` that calls `context.loaderOverlay.show(widgetBuilder: (*) => AppLoading())`and`static void hide(BuildContext context)`that calls`context.loaderOverlay.hide()`. Import loader_overlay.
